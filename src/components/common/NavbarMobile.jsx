      ...
      {links.map((link) => {
        ...
        return (
          <Link
            key={link.name}
            to={`/${link.name}`}
            className={`pb-2 border-b-2 text-sm font-medium transition-colors duration-200 w-full  lg:bg-base-100 lg:hidden flex items-center justify-center lg:rounded-box z-1 p-2 lg:shadow ${
              isActive
